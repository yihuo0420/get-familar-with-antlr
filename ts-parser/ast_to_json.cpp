...
namespace cyclone::parser
{
    ...
    Json AstToJson::visit(ParseTree *tree)
    {
        ...
        if (auto ruleNode = dynamic_cast<ParserRuleContext *>(tree))
        {
            ...
            SubTreeRoot subTreeRoot{CommonDef::ruleName(ruleNode->getRuleIndex()),
                                    std::get<0>(positionRange), std::get<1>(positionRange)};
            auto jsonifySubTreeRoot = Json(subTreeRoot);
            JsonList jsonifyChildren;
            for (const auto &child : tree->children)
                jsonifyChildren.push_back(visit(child));
            jsonifySubTreeRoot["SubPhases"] = jsonifyChildren;
    ...
    Json AstToJson::visitTerminal(TerminalNode *node)
    {
        ...
        {
            Terminal jsonifyTerminal = {CommonDef::tokenType(token->getType()),
                                        token->getStartIndex(),
                                        token->getStopIndex(),
                                        token->getText()};
