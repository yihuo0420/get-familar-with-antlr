...
expr returns [double calculated]   locals[string name] : expr Star expr #BBB
    |  expr Plus expr {$calculated = false;}  #CC 
    | OpenPar expr ClosePar #EDDD
    | <assoc = right> expr QuestionMark expr Colon expr #FDF
    | <assoc = right> expr Equal expr #GGG
    | identifier = id #EWF
    | flowControl  #EEF
    | INT #GGGGGGG
    |  String #FGF
